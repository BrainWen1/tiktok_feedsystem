...
// Upload 处理视频上传逻辑
func (s *VideoService) Upload(userID uint, fh *multipart.FileHeader) (string, error) {
	...
	default:
		log.Printf("Invalid file type in Upload Video: %v", ext)
	...
	if err := os.MkdirAll(dir, 0o755); err != nil {                                          // 创建目录，如果目录已存在则不会报错
		log.Printf("Failed to create directory in Upload Video: %v", err)
	...
	if err != nil {
		log.Printf("Failed to generate random filename in Upload Video: %v", err)
