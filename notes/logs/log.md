...
### 9.7
增加对用户未点赞视频的空占位缓存
list_liked接口：列出指定用户的喜欢列表；
修复点赞集合缓存的ttl设置bug，修复视频详情的是否点赞字段的bug。

### 9.8
VideoList接口也增加了对用户未点赞视频的空占位缓存，避免缓存穿透。

### 9.9
开始开发comment评论模块，完成publish发布评论接口，接入redis缓存和mq异步消费做削峰处理。
...
### 9.20
开始开发social模块，完成follow、unfollow接口，皆采用同步落库方案，保证数据一致性。

### 9.21
写了一会获取关注列表和粉丝列表这两个接口，但是还没写完，思绪有点乱。明天再写。